...
2. 技术术语规范统一，首次出现给出全称
3. 不编造具体实验数据，但应给出合理的性能指标范围和量化参数
4. 技术方案必须具体可实施，包含关键参数、阈值、判据，避免空泛描述
5. 使用 Markdown 格式输出，章节标题用 "## 一、xxx" 形式
6. 直接输出章节内容，不要添加任何额外说明或前缀
"""
...
GROUP_SPECS = [
    ...
    },
    {
        ...
        "instruction": (
            ...
            "每步包含：步骤名称、具体实现方法、关键参数/阈值/公式、与前后步骤的衔接。"
            "这是全文核心，必须详细具体，至少8段。）"
        ),
        "max_tokens": 4096,
    },
    {
        ...
        "instruction": (
            "请输出以下章节：\n"
            "## 六、有益效果（与现有技术对比至少4点，尽量量化并引用效果参考中的规范表述和标准）\n"
            "## 七、附图说明（列出4-6幅建议附图及内容说明）\n"
            "## 八、具体实施方式（按大纲的 embodiments 写至少2个详细实施例，"
            "每个实施例包含完整实施步骤、具体参数取值、工作流程，总篇幅不少于1500字）"
        ),
        "max_tokens": 4096,
    },
    {
