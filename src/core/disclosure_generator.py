...
4. 【理论公式】技术方案和具体实施方式中应给出必要的理论公式（属于公知理论/标准公式，不属编造），
   如材料蠕变本构方程、寿命预测公式（Larson-Miller参数法）、相对偏差计算式、分级决策判据等。
   公式必须用 LaTeX 格式书写，并用 $...$ 或 \\(...\\) 包裹（如 $\\varepsilon_c = A\\sigma^n\\exp(-Q/RT)$），
   不要输出裸 LaTeX 源码；公式后要逐项说明各符号含义。
5. 具体实施方式采用"示例性实施方式"框架：描述技术方案如何实施（步骤、流程、模块连接关系），
   需要参数的场合用"示例性取值，本领域技术人员可根据实际工况调整"表述；
   需要用户提供真实数据的场合用【此处补充实际数据】占位符标注
6. 技术方案要具体可实施（说清步骤和流程），但不得用编造的数值填充
7. 使用 Markdown 格式输出，章节标题用 "## 一、xxx" 形式
8. 直接输出章节内容，不要添加任何额外说明或前缀
"""
...
GROUP_SPECS = [
    {
        "key": "G1",
        "title": "发明名称 + 技术领域 + 背景技术",
        "instruction": (
            "请输出以下章节（使用标准专利格式，章节标题不带数字编号）：\n"
            "## 发明名称（一句话，≤25字，居中显示）\n"
            "## 技术领域（[0001] 开头，说明所属技术领域和具体对象）\n"
    ...
    },
    {
        ...
        "instruction": (
            "请输出以下章节（使用标准专利格式，章节标题不带数字编号）：\n"
            "## 发明内容（包含三部分，用【要解决的技术问题】【技术方案】【有益效果】三个小标题区分）：\n"
            "【要解决的技术问题】明确本发明的目的和要解决的现有技术缺陷；\n"
            "【技术方案】先概述整体技术路线，然后按大纲的 solution_steps 逐步展开，"
    ...
    },
    {
        ...
        "instruction": (
            "请输出以下章节（使用标准专利格式，章节标题不带数字编号）：\n"
            "## 附图说明（列出4-6幅建议附图，每幅图先写'图N：图名说明'，"
            "再给出该图的 ```mermaid flowchart 代码块（描述系统架构框图或方法流程图，"
            "节点用 A[中文说明]，连接用 -->，至少3个节点）。例如：\n"
            "图1：本发明系统整体架构示意图\n"
            "```mermaid\nflowchart TD\n"
            "    A[检测模块] --> B[仿真模块]\n"
            "    B --> C[AI代理模块]\n"
            "```\n"
            "每幅图都必须有 mermaid 代码块，方便自动生成附图）\n"
            "## 具体实施方式（按大纲的 embodiments 写至少2个示例性实施例，"
    ...
    },
    {
        ...
        "instruction": (
            "请输出以下章节（使用标准专利格式，章节标题不带数字编号）：\n"
            "## 权利要求书（建议）\n"
            "权利要求用标准数字编号逐条书写（格式：'1. 一种...'，每条独立一行，"
            "不要用'**权利要求1.**'加粗格式，不要用'权利要求1'文字开头），"
...
class DisclosureGenerator:
    ...
    def _assemble(self, sections: Dict[str, str]) -> str:
        """拼装全文并重排段落编号，输出标准专利文件格式

        结构：发明名称（无编号居中）→ 摘要（无编号）→ 权利要求书（编号）
              → 说明书（技术领域 / 背景技术 / 发明内容 / 附图说明 / 具体实施方式）
        """
        # 按 "## 标题" 切分各组的生成文本，得到章节块
        blocks = {}
        for g in ["G1", "G2", "G3", "G4"]:
            ...
            for part in re.split(r"\n(?=##\s)", text):
                ...
                if m:
                    ...
                    blocks.setdefault(title, part)

        def get_block(name: str) -> str:
            """按前缀匹配章节块（兼容标题带括号说明的情况）"""
            for k, v in blocks.items():
                if k == name or k.startswith(name + "（") or k.startswith(name + "("):
                    return v
            return ""

        patent_title = get_block("发明名称")
        abstract = get_block("摘要")
        claims = get_block("权利要求书")

        # 说明书五部分
        spec_parts = []
        for name in ["技术领域", "背景技术", "发明内容", "附图说明", "具体实施方式"]:
            b = get_block(name)
            if b:
                spec_parts.append(b)
        ...
        body = "\n\n".join(parts) if parts else "\n\n".join(
            sections[k] for k in ["G1", "G2", "G3", "G4"] if sections.get(k))
        return _renumber_paragraphs(body)
